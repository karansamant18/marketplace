...
import 'package:flutter_mobile_bx/widgets/marquee/fading_edge.dart';
...
class Marquee extends StatefulWidget {
  ...
  final List<HomeQuoteModel>? quoteModel;
...
class _MarqueeState extends State<Marquee> with SingleTickerProviderStateMixin {
  ...
  void _initialize(BuildContext context) {
    ...
    final totalLength = 3786.639892578125 + widget.blankSpace;
    ...
    _startPosition = 2 * decelerationLength - widget.startPadding;
  ...
  Widget build(BuildContext context) {
    ...
    Widget marquee = ListView.builder(
      ...
      itemBuilder: (_, i) {
            ...
            builder: (context, snap) {
              if (snap.hasData && snap.data != null) {
                if (snap.data!.exchangeInstrumentID ==
                    widget.quoteModel![i].symphonyToken) {
                  widget.quoteModel![i].ltp = snap.data!.lastTradedPrice!;
                  widget.quoteModel![i].changePct = snap.data!.percentChange!;
                }
              }
              return Padding(
                padding:
                ...
                child: Align(
                  ...
                  child: Row(
                    children: [
                      ...
                      Text(
                        "${widget.quoteModel?[i].ltp.toStringAsFixed(2)}",
                        style: TextStyle(
                          color: ((widget.quoteModel?[i].changePct ?? 0.0) < 0)
                      ...
                      // const SizedBox(width: 5),
                      ((widget.quoteModel?[i].changePct ?? 0.0) < 0)
                          ? const Icon(
                      ...
                      Text(
                        "${((widget.quoteModel?[i].changePct.toStringAsFixed(2) ?? 0.0))}%",
                        style: TextStyle(
                          color: ((widget.quoteModel?[i].changePct ?? 0.0) < 0)
                              ? ConstColor.pink62Color
                              : ConstColor.greenColor75,
        ...
        // final text = i.isEven
        //     ? Text(
        //         widget.quoteModel?[i].symbol ?? 'zsz',
        //         style: TextStyle(
        //           color: ConstColor.whiteColor,
        //         ),
        //         textScaleFactor: widget.textScaleFactor,
